...
### 2.4 已接受的代价：列表页的浏览量会滞后（2026-09-19 明确）

`GET /article/list`（`selectArticlesList`）**读的是 DB 的 `view_count`**，而 DB 只在缓存 miss 时（约 30 分钟一次）被回写，所以：

- **同一个人在两个页面看到的数字会不一样** —— 详情页读 Redis 计数器（实时），列表页读 DB（滞后）
- 差距可以很可观：一篇被刷了 1 万次的文章，列表页可能还显示几百
- 但**不会永久错**：下一次 miss 回写后自动收敛，不存在数据丢失

**决策：接受这个滞后**，不做额外同步。两条理由：

1. 列表页的浏览量本来就只是"热度参考"，用户不会拿它跟详情页逐一对账；
2. 要让列表也实时，就得对**每一篇**文章再读一次 Redis（`MGET`）—— 等于把一次 DB 分页查询换成一次 Redis 批量查询，换来"数字好看一点"，代价是**列表接口从此强依赖 Redis**。而现在 `selectArticlesList` **完全不碰 Redis**，Redis 挂掉时列表照常可用。

> 面试被问"缓存和 DB 不一致怎么办"时，这条正是**"按业务容忍度决定一致性级别"**的实例 —— 不是所有不一致都值得修。要看的是：**错的后果是什么、错多久、修它要付出什么**。
